...
Скрипт `yarn run test:e2e` объявлен в `package.json` (конфиг `test/jest-e2e.json`), но каталог `test/` и e2e-тесты пока **не созданы**. Добавление e2e-тестов — открытая задача.
