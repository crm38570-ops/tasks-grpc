...
| `JWT_EXPIRES_IN` | да | — | Срок жизни токена. Формат — как в `jsonwebtoken`/`ms`: число (трактуется как секунды) или строка, для которой время вычисляется через `ms()`: с единицами (`3600s`, `1d`) или без (тогда как миллисекунды, например `3600` = 3.6 c). В `.env.stage.dev` и docker-compose используется `3600s` (= 1 час) |
...
- В рабочей копии лежат образцы: `.env.stage.dev` (локальная разработка) и пустой `.env.stage.prod` (зарезервирован под продакшен — заполните его перед деплоем).
