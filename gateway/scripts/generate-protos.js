...
for (const proto of findProtos(protoRoot)) {
  const moduleDir = path.dirname(proto);
  const outDir = path.join(serviceRoot, 'src', 'proto', path.basename(moduleDir), 'generated');
  ...
  execFileSync(
    ...
    [
      ...
      `--proto_path=${include}`,
      rel(path.join(moduleDir, path.basename(proto))),
    ],
    { stdio: 'inherit' },
  );
